 ...
 *
 */
class MIpValidator extends CRegularExpressionValidator
{
    /**
     * @var string the IP Protocol version to validate against
     */
    private $version = 'both';

    public function setVersion($version)
    {
        $this->version = strtolower($version);
        if (!in_array($this->version, array('ipv4', 'ipv6', 'both')))
            throw new CException(Yii::t(__CLASS__, 'Invalid IP version: {value}.', array('{value}' => $this->version)));
    }
    ...
    protected function validateAttribute($object, $attribute)
    {
        ...
        $flags = array();
        if ($this->version === 'ipv4' || $this->version === 'both') {
            $flags[] = FILTER_FLAG_IPV4;
        }
        if ($this->version === 'ipv6' || $this->version === 'both') {
