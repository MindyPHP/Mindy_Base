...
use Mindy\Base\Interfaces\IWebUser;
 ...
 * @since 1.0
 */
class WebUser extends ApplicationComponent implements IWebUser
{
        ...
        if ($this->beforeLogin($id, $states, false)) {
            ...
            if ($duration > 0) {
                if ($this->allowAutoLogin)
                    ...
                    throw new Exception(Mindy::t('yii', '{class}.allowAutoLogin must be set true in order to use cookie-based authentication.',
    ...
    {
        if ($defaultUrl === null) {
            $defaultReturnUrl = Mindy::app()->getUrlManager()->showScriptName ? Mindy::app()->getRequest()->getScriptUrl() : Mindy::app()->getRequest()->getBaseUrl() . '/';
        } else {
    ...
    public function loginRequired()
    {
        ...
        throw new HttpException(403, Mindy::t('yii', 'Login Required'));
    }
    ...
    public function getStateKeyPrefix()
    {
        if ($this->_keyPrefix !== null)
            ...
            return $this->_keyPrefix = md5('Yii.' . get_class($this) . '.' . Mindy::app()->getId());
    }
    ...
    {
        Mindy::app()->getSession()->regenerateID(true);
        $this->setId($id);
        $this->setName($name);
    ...
    public function checkAccess($operation, $params = array(), $allowCaching = true)
    {
        ...
        $access = Mindy::app()->getAuthManager()->checkAccess($operation, $this->getId(), $params);
