...
class FilterChain extends BaseList
{
    ...
    public function run()
    {
        if ($this->offsetExists($this->filterIndex)) {
            ...
            Mindy::trace('Running filter ' . ($filter instanceof InlineFilter ? get_class($this->controller) . '.filter' . $filter->name . '()' : get_class($filter) . '.filter()'), 'system.web.filters.FilterChain');
            $filter->filter($this);
        } else
