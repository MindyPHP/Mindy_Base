...
use Mindy\Base\Enumerable;
use Mindy\Base\PropertyValue;
...
class CPropertyValueTest extends CTestCase
{
    public function testEnsureBoolean()
    {
        ...
        foreach ($entries as $index => $entry)
            $this->assertTrue(PropertyValue::ensureBoolean($entry[0]) === $entry[1],
    ...
    public function testEnsureString()
    {
        ...
        foreach ($entries as $index => $entry)
            $this->assertTrue(PropertyValue::ensureString($entry[0]) === $entry[1],
                "Comparison $index: {$this->varToString($entry[0])}=={$this->varToString($entry[1])}");
    }
    ...
    public function testEnsureInteger()
    {
        ...
        foreach ($entries as $index => $entry)
            $this->assertTrue(PropertyValue::ensureInteger($entry[0]) === $entry[1],
                "Comparison $index: {$this->varToString($entry[0])}=={$this->varToString($entry[1])}");
    }
    ...
    public function testEnsureFloat()
    {
        ...
        foreach ($entries as $index => $entry)
            $this->assertTrue(PropertyValue::ensureFloat($entry[0]) === $entry[1],
                "Comparison $index: {$this->varToString($entry[0])}=={$this->varToString($entry[1])}");
    }
    ...
    public function testEnsureArray()
    {
        ...
        foreach ($entries as $index => $entry)
            $this->assertTrue(PropertyValue::ensureArray($entry[0]) === $entry[1],
                "Comparison $index: {$this->varToString($entry[0])}=={$this->varToString($entry[1])}");
    }
    ...
    {
        $obj = new stdClass;
        $this->assertTrue(PropertyValue::ensureObject($obj) === $obj);
    }

    public function testEnsureEnum()
    {
        $this->assertTrue(PropertyValue::ensureEnum('Left', 'TextAlign') === 'Left');
